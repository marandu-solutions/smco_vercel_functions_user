...
export default allowCors(async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  validateJWT(request, response, async (currentUser: CurrentUser) => {
    ...
    try {
      updateAppointmentData = updateAppointment.parse({
        ...request.body,
    ...
    try {
      const { id, ...updateData } = updateAppointmentData;
      const appointment = await xata.db.appointment.update(id, updateData);
      return response.status(200).json({ message: "Appointment created successfully", appointment: appointment });
    } catch (error) {
      return response.status(500).json({ message: "Failed to create appointment", error });
    }
  });
});
