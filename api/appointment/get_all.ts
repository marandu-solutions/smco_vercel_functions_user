...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  await allowCors(request, response);
  ...
  validateJWT(request, response, async (currentUser: CurrentUser) => {
    ...
    try {
      const result = await xata.db.appointment.select([
        ...
        "previous_appointment.xata_id", "xata_createdat", "updated_by.name", "xata_updatedat"
      ...
      return response.status(500).json({ message: "Failed to fetch appointments", error: error.message || "Unknown error" });
    }
  });
}
