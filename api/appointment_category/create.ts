...
import { createAppointmentCategory } from "../../schemas/appointment_category";
...
export default allowCors(async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  validateJWT(request, response, async (currentUser: CurrentUser) => {
    let appointmentCategoryData;
    try {
      appointmentCategoryData = createAppointmentCategory.parse({ ...request.body, ubs: currentUser.ubs });
    } catch (error) {
    ...
    try {
      const appointmentCategory = await xata.db.appointment_category.create(appointmentCategoryData);
      return response.status(200).json({ message: "Appointment category created successfully", news: appointmentCategory });
    } catch (error) {
